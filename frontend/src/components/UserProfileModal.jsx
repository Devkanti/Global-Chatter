...
export default function UserProfileModal({ isOpen, onClose, targetUser, currentUser, userProfiles, userStatuses, userFriends, friendRequests, sentRequests = new Set(), onSendRequest, onCancelRequest, userPrivacyMode, onJoinRoom }) {
  ...
  return (
    <div className="modal-overlay">
      <div className="modal-content animate-fade-in" style={{ background: 'var(--panel-bg)', backdropFilter: 'blur(24px)', borderRadius: '24px', border: '1px solid var(--panel-border)', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.8), 0 0 0 1px var(--card-bg)', width: '100%', maxWidth: '400px', padding: '3rem 2rem', position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        <button onClick={onClose} style={{ position: 'absolute', top: '1.25rem', right: '1.25rem', color: 'var(--text-muted)', cursor: 'pointer', background: 'var(--hover-bg)', border: 'none', padding: '0.5rem', borderRadius: '50%', display: 'flex', transition: 'all 0.2s' }} onMouseEnter={e => { e.currentTarget.style.background = 'var(--hover-bg-strong)'; e.currentTarget.style.color = 'var(--text-main)'; }} onMouseLeave={e => { e.currentTarget.style.background = 'var(--hover-bg)'; e.currentTarget.style.color = 'var(--text-muted)'; }}>
        ...
        <div style={{ position: 'relative', marginBottom: '1.5rem' }}>
          ...
          <div style={{ position: 'absolute', bottom: '2px', right: '2px', width: '22px', height: '22px', borderRadius: '50%', background: dotColor, border: '3px solid var(--panel-bg)', boxShadow: targetStatus !== 'invisible' ? `0 0 12px ${dotColor}80` : 'none' }}></div>
        </div>
        ...
        {!isSelf && (
          <div style={{ display: 'flex', gap: '1rem', width: '100%' }}>
            ...
            <button 
              ...
              style={{ flex: 1, padding: '0.75rem', borderRadius: '12px', background: 'var(--hover-bg)', color: 'var(--text-main)', border: '1px solid var(--panel-border)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', fontWeight: '600', transition: 'all 0.2s', cursor: 'pointer' }}
